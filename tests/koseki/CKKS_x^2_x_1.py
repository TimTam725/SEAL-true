...
def Homo_Stochastic_test():
    ...
    error = []
    ave_error=[]
    # for loopcnt in tqdm([1,2,5,10,20,50,100,200,500,1000]):
    ...
    # for loopcnt in tqdm([1]):
        ...
        for m_i in tqdm(plainlist):
            ...
            for _ in range(loopcnt):
                ...
                # print("    + Exact scale in x^2: " + "%.10f" % sqar_encrypted.scale())
                # print("    + Exact scale in   x: " + "%.10f" % x_encrypted.scale())
                # print("    + Exact scale in   1: " + "%.10f" % plain_one.scale())

                sqar_encrypted.set_scale(pow(2.0, 40))
                
                # print("-" * 50)
                # print("Normalize encryption parameters to the lowest level.")
                last_parms_id = sqar_encrypted.parms_id()
                evaluator.mod_switch_to_inplace(x_encrypted, last_parms_id)
                evaluator.mod_switch_to_inplace(plain_one, last_parms_id)

                # encrypted_result = Ciphertext()
                evaluator.add_inplace(sqar_encrypted, x_encrypted)
                evaluator.add_plain_inplace(sqar_encrypted, plain_one)
                ...
                decryptor.decrypt(sqar_encrypted, plain_result)
